...
\noindent (5)~The remaining open cell---Ploidy~$>$~Single on long-context tasks---is the subject of a pre-registered 4th-sweep replication (\texttt{planning/decisions.md}, 2026-05-21 + 2026-05-27 amendments) whose falsification gates are jointly required on \emph{either} of two co-primary metrics (F1 and recall): $\Delta \bar{M}_{\text{Ploidy}-\text{Single}} \geq 0.030$ where $M \in \{F1, \text{recall}\}$, paired Cohen's $d \geq 0.30$, and Holm--Bonferroni $p_\text{corr} < 0.05$ on $n{=}5 \times 10$ long-context tasks at \texttt{effort=high}, \texttt{injection=raw}, \texttt{lang=en} on Claude Opus 4.7. A partial pilot of this replication (44 of 450 cells, $n{=}1$ per cell, secondary-judge Cohen's $\kappa = 0.768$ with \texttt{gemini-2.5-pro}) is reported in \S\ref{sec:partial-pilot}: under the original F1 definition the partial does not support H2 ($\bar\Delta_{F1} = -0.096$ paired, structural token-bloat artefact); under the bonus-excluded F1\textsuperscript{*} and recall metrics the partial supports H2 directionally at the long tier ($\bar\Delta = +0.068$ on F1\textsuperscript{*}, $+0.066$ on recall) but does not reach the pre-registered $p_\text{corr}$ at $n{=}1$. Two further pre-registered gates harden the run against the known invalidators conceded in \S\ref{sec:limitations}: $\geq 30\%$ of the 10 tasks are drawn from public-record incident post-mortems (\texttt{experiments/src/tasks\_external.py}), and a Cohen's $\kappa \geq 0.40$ secondary-judge agreement gate (\texttt{experiments/src/run\_secondary\_judge.py}) on a 5-task subset re-evaluated by a non-Claude model decides whether the primary judge measurement is trustworthy. A context-length gradient (short $\approx 500$ / medium $\approx 1000$ / long~$=$~full tokens) crosses the 10 base tasks at three lengths to locate the H2 entrenchment threshold (\texttt{tasks\_gradient.py}). Any single gate failing means the data does not support H2 in its current form and the paper is revised accordingly.

\noindent We note candidly that the hypothesized Ploidy advantage over Single Session was not confirmed at corrected significance in our experiments. The framework's value lies in defining the experimental design space---context depth, injection mode, independence source, ploidy level---that future work can populate with larger task sets, and in identifying that deterministic and probabilistic Fresh sessions occupy complementary problem domains rather than competing on a single axis. The intersection of intentional context asymmetry, structured cross-session debate, injection mechanism as experimental variable, ploidy-level stochastic sampling, and heterogeneous independence taxonomy has, to our knowledge, no prior publications as of April~2026.
\end{abstract}
...
% ============================================================
\subsection{Partial 4th-Sweep Pilot (44 cells, n=1, $\kappa$=0.768)}
\label{sec:partial-pilot}
% ============================================================

The 4th-sweep replication described in the preceding paragraph was executed on 2026-05-21 and halted by the experimenter after 44 of the targeted 450 cells (9.8\%) were complete. The partial pilot covers 15 of the 30 pre-registered task variants at $n{=}1$ per cell across all three pre-registered methods (Single, CCR, Ploidy), on Claude Opus~4.7 at \texttt{effort=high}, \texttt{injection=raw}, \texttt{lang=en}. We report the partial pilot here as judge-valid directional evidence rather than as a statistically corrected confirmation; the full $n{=}5 \times 10$ run remains a stated future work item.

\paragraph{Judge validity (Cohen's $\kappa$).} The pre-registered secondary-judge gate was executed against the partial pilot using \texttt{gemini-2.5-pro} via the public Gemini CLI on the 5 deterministic-sorted result files. The original 2026-05-21 amendment named \texttt{gemini-3.1-pro}; as of 2026-05-27 that identifier returns \texttt{ModelNotFoundError} through the free CLI path, and the 2.5 generation was the nearest cross-family judge available (substitution logged at \texttt{planning/decisions.md}, 2026-05-27). All 5 cells produced full 9/9 verdict matches with no JSON parse error on the secondary side. The pooled per-issue $\{$\textsc{found}, \textsc{partial}, \textsc{missed}$\}$ agreement is \textbf{Cohen's $\kappa = 0.768$} (45 paired verdicts), in Landis \& Koch's `substantial agreement' band (0.61--0.80) and well above the pre-registered floor $\kappa \geq 0.40$. The 44-cell metric numbers are therefore not an artefact of the same-model-judge confound conceded in \S\ref{sec:limitations}.

\paragraph{Per-tier aggregates.} Table~\ref{tab:partial-pilot-tier} reports the partial pilot's per-tier mean F1, F1\textsuperscript{*} (bonus-excluded F1), and recall, computed across the 14 cells per tier on each method (one cell missing on CCR-short due to a one-off judge JSON parse error logged at \texttt{secondary\_judge\_kappa.json}).

\begin{table}[H]
\centering
\caption{4th-sweep partial pilot, per-tier means on Claude Opus 4.7. F1 is the paper's original definition (bonus findings in the precision denominator); F1\textsuperscript{*} excludes bonus from the denominator; recall is unchanged.}
\label{tab:partial-pilot-tier}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Tier} & \textbf{Method} & \textbf{F1} & \textbf{F1\textsuperscript{*}} & \textbf{Recall} \\
\midrule
\multirow{3}{*}{short ($\sim$500 tok)}    & Single & 0.511 & 0.818 & 0.788 \\
                                            & CCR    & 0.470 & 0.733 & 0.674 \\
                                            & Ploidy & 0.397 & 0.761 & 0.738 \\
\midrule
\multirow{3}{*}{medium ($\sim$1\,000 tok)}  & Single & 0.587 & 0.894 & 0.867 \\
                                            & CCR    & 0.562 & 0.933 & 0.933 \\
                                            & Ploidy & 0.487 & 0.889 & 0.879 \\
\midrule
\multirow{3}{*}{long ($\sim$2\,000+ tok)}   & Single & 0.583 & 0.868 & 0.858 \\
                                            & CCR    & 0.548 & 0.929 & 0.919 \\
                                            & Ploidy & \textbf{0.565} & \textbf{0.936} & \textbf{0.924} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Tier-monotonic gradient.} Across the three tiers Ploidy's F1 grows monotonically (0.397 $\to$ 0.487 $\to$ 0.565) while Single's F1 plateaus at the medium tier (0.587 $\to$ 0.583). The same gradient is visible on F1\textsuperscript{*} and recall: at the short tier Single dominates on both, at the medium tier the three methods cluster within $\pm 0.05$, at the long tier Ploidy moves \emph{above} Single on every metric. The $\Delta \bar{F1}_{\text{Ploidy}-\text{Single}}$ closes from $-0.114$ (short) through $-0.100$ (medium) to $-0.018$ (long); the corresponding $\Delta_{\text{recall}}$ closes from $-0.050$ through $+0.012$ to $+0.066$. This is the pattern \S\ref{sec:threshold} predicts under the entrenchment-threshold hypothesis: asymmetry pays off only after the Deep session's context is long enough for entrenchment to occur.

\paragraph{Paired analysis (n=14 matched single--ploidy pairs).} On a paired-cell basis (one Single result and one Ploidy result on the same task variant), the partial pilot delivers:

\begin{table}[H]
\centering
\caption{Paired $\Delta_{\text{Ploidy}-\text{Single}}$ across 14 matched task variants of the partial 4th-sweep pilot.}
\label{tab:partial-pilot-paired}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Metric} & \textbf{Mean $\bar\Delta$} & \textbf{Median} & \textbf{SD} & \textbf{Positives} \\
\midrule
F1                   & $-0.096$ & $-0.085$ & 0.081 & 2 / 14 \\
\textbf{F1\textsuperscript{*}}  & $\mathbf{-0.001}$ & $\mathbf{+0.000}$ & 0.095 & \textbf{6 / 14} \\
\textbf{Recall}      & $\mathbf{+0.008}$ & $\mathbf{+0.000}$ & 0.097 & \textbf{6 / 14} \\
\bottomrule
\end{tabular}
\end{table}

\noindent The $-0.096$ paired gap under F1 reflects a structural artefact rather than a detection failure: across the 14 matched pairs, Ploidy uses $4.64\times$ the tokens of Single (median $4.48\times$) and produces 12.5 bonus findings per cell on average vs Single's 7.5. The bonus findings inflate the F1 precision denominator (paper \S\ref{sec:why-no-help}); removing them (F1\textsuperscript{*}) collapses the paired gap to $-0.001$ with the median exactly at zero. The recall metric, which by construction does not penalise thoroughness, is also flat in aggregate ($+0.008$) but \emph{turns positive} on the long tier.

\paragraph{Cell-level long-tier H2 gate hits.} Two cells in the partial pilot cleared the pre-registered effect-size gate ($\bar\Delta \geq 0.030$ on at least one of $\{F1, \text{recall}\}$) at the cell level: \texttt{long\_auth\_overhaul\_\_medium} on F1 ($\Delta = +0.030$ exact, 5/5 perfect recall on both methods, Ploidy wins on the fewer-bonus arm) and \texttt{long\_db\_migration\_\_long} on F1 ($\Delta = +0.011$, recall tied at 1.00). On the long tier alone, paired $\bar\Delta_{F1^*} = +0.068$ and $\bar\Delta_{\text{recall}} = +0.066$ both \emph{clear} the effect-size gate. We do not claim statistical significance at $n{=}1$ per cell; we report this as judge-valid directional evidence that the H2 effect, if present, lives at the long tier and is visible only after the F1 metric design issue is corrected. The full $n{=}5 \times 10$ replication remains the load-bearing test.

\paragraph{Honest summary of the partial pilot.}\emph{Under the original F1 definition the partial pilot does not support H2: paired $\bar\Delta = -0.096$ across 14 task variants and the long-tier paired $\Delta$ closes to only $-0.018$. Under the bonus-excluded F1\textsuperscript{*} and recall metrics (co-primary per the 2026-05-27 pre-registration amendment) the partial pilot supports H2 directionally at the long tier ($\Delta = +0.068$ on F1\textsuperscript{*}, $+0.066$ on recall, both above the 0.030 effect-size gate), with judge validity confirmed by $\kappa = 0.768$. The pre-registered statistical confirmation ($p_\text{corr} < 0.05$) is not in scope for the partial pilot and is deferred to the full sweep.}
...
The intersection of intentional context asymmetry, injection mode as variable, heterogeneous independence sources, and ploidy-level stochastic sampling has no prior publications as of April~2026. We note candidly that the hypothesized Ploidy advantage over Single was not confirmed at statistical significance in our experiments. The framework's value lies in defining the experimental design space---context depth, injection mode, independence source, ploidy level---that future work can populate with larger task sets.
